...
const extraSheetSchema = z.object({
...
const inputSchema = z.object({
  ...
  rows: z.array(z.record(z.string(), z.unknown())),
  extraSheets: z.array(extraSheetSchema).max(10).optional()
});

const outputSchema = z.object({
  csvPublic: z.string().url(),
  mdPublic: z.string().url(),
  xlsxPublic: z.string().url()
});
...
const tool: McpTool<z.infer<typeof inputSchema>, z.infer<typeof outputSchema>> = {
  contract: {
    ...
    description:
      "Vygeneruje CSV, Markdown a Excel (.xlsx) z dat a nahraje je do Supabase Storage. Volitelne extraSheets pro vice listu v Excelu.",
    inputSchema,
    outputSchema,
    auth: "service-role",
    sideEffects: ["Storage upload (CSV + MD + XLSX) do bucketu env.SUPABASE_STORAGE_BUCKET"]
  },
