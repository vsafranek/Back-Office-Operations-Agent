...
- Runtime mode: default **custom styled PPTX** via **`pptxgenjs`** with **`defineSlideMaster`** (společné pozadí, záhlaví, akcenty) a **typovaný obsah** z LLM (`title` | `content` | `stats` | `quote` | `section`) — render podle typu v `lib/agent/tools/presentation-typed-deck.ts`. Legacy **external template** (`pptx-automizer` + `assets/presentation-templates/blue-white-company-profile.pptx`) only when `PRESENTATION_USE_TEMPLATE=true` and the file exists; obsah se převádí na `{ title, bullets }`.
